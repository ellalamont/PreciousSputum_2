...
final_volcano
# ggsave(final_volcano,
#        file = "W2.cure.ComparedTo.W2.Relapse_BobDESeq2_Run1to3_v1.pdf",
...
df_DESeq_inR <- res_W2Relapse_vs_W2Cure_df %>% 
  dplyr::select(gene, log2FoldChange, DE2, padj) %>% 
  rename(LOG2FOLD_DESeq_inR = log2FoldChange,
         DE2_DESeq_inR = DE2,
         GENE_ID = gene,
         padj_DESeq_inR = padj) 
df_DESeq_Bob <- DESeq_W2.relapse.ComparedTo.W2.cure %>% 
...
my_plot <- df_combined %>% drop_na() %>%
  ...
  scale_color_manual(values = c("#999990", "blue1", "maroon4")) + 
  labs(title = "W2 Relapse vs W2 Cure DEG Log2Fold=2 >60%TxnCov",
...
my_plot
# ggsave(my_plot,
#        file = paste0("W2.RelapseVsCure_AdjP_v1.pdf"),
...
my_plot
# ggsave(my_plot,
#        file = paste0("W2.RelapseVsCure_ogP_v1.pdf"),
#        path = "Figures/DEG_Comparisons_60TxnCov",
#        width = 9, height = 6, units = "in")
...
df_DESeq_inR <- res_W2Relapse_vs_W2Cure_df %>% 
  ...
  rename(LOG2FOLD_DESeq_inR = log2FoldChange,
         ...
         GENE_ID = gene,
         padj_DESeq_inR = padj) 
df_metaresults <- list_dfs_f2$W2.relapse.ComparedTo.W2.cure %>% 
...
my_plot
# ggsave(my_plot,
#        file = paste0("W2.RelapseVsCure_MetaResults_ogP_v1.pdf"),
#        path = "Figures/DEG_Comparisons_60TxnCov",
#        width = 9, height = 6, units = "in")
